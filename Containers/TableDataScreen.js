...
import {Modal,TouchableHighlight, ActivityIndicator,Platform,DeviceEventEmitter,NativeEventEmitter,ToastAndroid,StyleSheet, Switch,Button, Image, ImageBackground, Text, View, Dimensions, TextInput, TouchableOpacity, ScrollView } from 'react-native';
        ...
        return (
           ...
           {/* <Text  style={styles.title}>Connected:<Text style={{color:"blue"}}>{!this.state.name ? 'No Devices' : this.state.name}</Text></Text> */}
                ...
                <Row size={13}>
                    <ScrollView>
                        {/* turn on bluetooth */}
                        <Switch style={[styles.switch]} value={this.state.bleOpend} onValueChange={(v)=>{this.setState({loading:true})
                        if(!v){
                            BluetoothManager.disableBluetooth().then(()=>{
                                this.setState({bleOpend:false,connected:false,loading:false,foundDs:[],pairedDs:[]});},(err)=>{alert(err)});
                        }else{BluetoothManager.enableBluetooth().then((r)=>{var paired = [];
                                if(r && r.length>0){
                        ...
                        <Row style={[styles.Col1]}>
                            <Text style={[styles.deviceTitle]}>{!this.state.name?"Disconnect":this.state.name}</Text>
                            <TouchableOpacity style={[styles.button]} onPress={() => this._scan()}>
                    ...
                    <Col style={[styles.menubottomShare]}>
                      <TouchableHighlight onPress={() => alert('image clicked')}>
                        <Image style={[styles.logoShare]} source={require('../assets/icons/viewdata/share.png')}></Image>
                      </TouchableHighlight>
                    </Col>
                    <Col style={[styles.menubottomSync]}>
                      <TouchableHighlight onPress={() => alert('image clicked')}>
                        <Image style={[styles.logoShare]} source={require('../assets/icons/viewdata/sync.png')}></Image>
                      </TouchableHighlight>
                    </Col>
                    <Col style={[styles.menubottomPrint]}>
                      <TouchableHighlight disabled={this.state.loading || this.state.boundAddress.length <= 0}
                             onPress={async () => {
                         try {
                            this.state.salt.map(sal => {
                            ...
                            let columnWidths = [14, 1, 17];
                             ...
                             BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER);
                             BluetoothEscposPrinter.printPic(base64PngLogo, {width: 220, left: 60});
                             BluetoothEscposPrinter.printText("\r\n\r\n\r\n", {});
                        })
                        } catch (e) {alert(e.message || "ERROR");
                    }
                    }}>
                        <Image style={[styles.logoShare]} source={require('../assets/icons/viewdata/print.png')}></Image>
                      </TouchableHighlight>
                    </Col>
                </Row>
                <Row>
